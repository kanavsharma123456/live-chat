...
import { PeerStats } from "@antmedia/webrtc_adaptor/js/peer_stats.js"
import { WebSocketAdaptor } from "@antmedia/webrtc_adaptor/js/websocket_adaptor.js"
...
export class WebRTCAdaptor {
	...
	*/
	...
	// switchFacingMode(streamId, facingMode = 'user') {
	// this.publishMode = 'camera';
	// var audioConstraint = false;
	// if (typeof this.mediaConstraints.audio != "undefined" && this.mediaConstraints.audio != false) {
	// 	audioConstraint = this.mediaConstraints.audio;
...
toggleVideoCam(){
	...
	switchVideoCameraCapture(streamId, deviceId, facingMode = 'user') {
		//stop the track because in some android devices need to close the current camera stream
		...
		this.mediaConstraints.video.facingMode = facingMode;
